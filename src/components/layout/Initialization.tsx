...
export function Initialization({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      if (visited || window.matchMedia("(prefers-reduced-motion: reduce)").matches) { setInitializationState("ready"); return; }
      setInitializationState("running");
      element.dataset.play = "true";
      finish = window.setTimeout(() => { delete element.dataset.play; setInitializationState("ready"); }, 1200);
    });
    return () => { cancelAnimationFrame(frame); window.clearTimeout(finish); if (element.dataset.play) setInitializationState("ready"); delete element.dataset.play; };
  }, []);
