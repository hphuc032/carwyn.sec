...
export function SecurityLog({ locale }: { locale: Locale }) {
  ...
  return <section id="log" className="security-log" aria-labelledby="security-log-title" tabIndex={-1}>
    <div className="security-log-inner">
      ...
      <div className="security-log-intro"><h2 id="security-log-title" data-reveal="chapter" data-reveal-key="log">{vi ? "Mở sổ ghi chép kỹ thuật." : "Open the technical notebook."}</h2><p>{vi ? "Ghi chép thực hành, giới hạn bằng chứng và những điều rút ra trong quá trình học." : "Practice notes, evidence boundaries, and lessons from technical study."}</p></div>
