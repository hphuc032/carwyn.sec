...
try {
  ...
  for (const locale of ["en", "vi"]) {
    ...
    for (const width of [375, 430, 768, 1024, 1440, 1920]) {
      ...
      assert.equal(await page.locator(".achievement-record").count(), 2);
      assert.equal(await page.locator(".achievement-group").count(), 2);
      const metrics = await page.locator("#achievements").evaluate(section => ({
        overflow: document.documentElement.scrollWidth > innerWidth,
        clipped: [...section.querySelectorAll("h2,h3,h4,p,span,strong")].some(element => element.scrollWidth > element.clientWidth + 1),
    ...
    assert.ok(!text.includes("Top 4"));
    assert.ok(!text.includes("CSCV"));
    assert.deepEqual(await page.locator(".achievement-index").allTextContents(), ["001", "002"]);
    assert.equal(await page.locator('#achievements a,#achievements button,#achievements [data-cursor],#achievements canvas,#achievements img').count(), 0);
    console.log(`PASS ${locale}: two published records, two non-empty categories, withheld competition claims`);
  }
  ...
  assert.equal(await staticPage.locator(".achievement-record").count(), 2);
  assert.ok((await staticPage.locator("#achievements").textContent()).includes("Đang học"));
